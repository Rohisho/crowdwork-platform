...
user_problem_statement: |
  Build "Spotted Jobs" — a hyperlocal, crowdsourced job discovery platform for the UK.
  Users snap photos of "We're Hiring" posters in shop windows; the AI extracts details,
  and the job appears on a live Mapbox map for nearby seekers.

  Iteration 1 scope (aha moment): image upload → OpenAI GPT-4o vision OCR + moderation →
  Supabase Storage + Postgres insert → Mapbox map + realtime feed + Supabase auth (magic link + Google).
...
backend:
  - task: "Health endpoint /api/health"
    ...
    status_history:
        - working: true
          agent: "main"
          comment: "Curl returned all env flags true (supabase_url/anon/service, mapbox, llm)."
...
frontend:
  - task: "Home page with Mapbox map + list toggle + realtime feed + FAB upload"
    implemented: true
    working: "NA"
    file: "app/page.js, components/map-view.jsx, components/upload-sheet.jsx, components/job-detail-sheet.jsx"
    stuck_count: 0
    priority: "high"
    needs_retesting: false
    status_history:
        - working: "NA"
          agent: "main"
          comment: "Not requested by user for automated test yet."

  - task: "Login page with magic link + Google OAuth"
    implemented: true
    working: "NA"
...
test_plan:
  current_focus:
    ...
    - "GET /api/jobs (nearby jobs via PostGIS RPC or list fallback)"
    - "POST /api/jobs/upload (image → AI vision extract → storage → insert)"
  stuck_tasks: []
  test_all: false
...
agent_communication:
    - agent: "main"
      message: |
        Iteration 1 backend implemented for Spotted Jobs. All env keys wired. Supabase schema.sql
        MUST be executed once by the user in the Supabase SQL editor before backend tests can pass
        (creates vacancies table, PostGIS, RLS, RPC, and job-posters storage bucket).

        Please test the following FIRST (all high-priority):
          1. GET  /api/health → expect ok:true and env flags true.
          2. GET  /api/jobs?lat=51.5074&lng=-0.1276&radius_m=10000 → 200 with jobs array (may be empty).
          3. GET  /api/jobs (no lat/lng) → 200 with jobs array.
          4. POST /api/jobs/upload without auth cookie → 401.
          5. POST /api/jobs/upload with a fake image + invalid lat/lng → 400.
          6. POST /api/jobs/upload with a non-poster public image (e.g. plain landscape) → 422
             with body.error mentioning "hiring poster" or "moderation", and body.ai present.

        For step 6 use a small JPEG that will not pass the AI check — this validates the AI pipeline
        without needing a real logged-in user or the schema being fully populated.

        Skip votes/bookmarks/reports for now — those need seeded rows and RLS-authenticated cookies;
        we'll cover them in iteration 2.
